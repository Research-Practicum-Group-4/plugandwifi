...
// Verified fallback that is kept separate from the per-venue image selection.
// Some historical Unsplash identifiers in the larger pools no longer resolve.
const RELIABLE_BACKUP_IMAGE = 'https://images.unsplash.com/photo-1497366216548-37526070297c?w=600';
...
export function getVenueImage(venue: Venue): string {
  ...
  return image || getVenueFallbackImage(venue);
}

export function getVenueBackupImage(venue: Venue): string {
  return getVenueFallbackImage(venue) === RELIABLE_BACKUP_IMAGE
    ? 'https://images.unsplash.com/photo-1554118811-1e0d58224f24?w=600'
    : RELIABLE_BACKUP_IMAGE;
}
