...
def get_venue_listing_column_status(connection):
    ...
    existing_columns = {column_name for (column_name,) in column_rows}
...
def migrate_venue_listing_fields(apply=False):
    with engine.connect() as connection:
        venue_count, existing_columns = get_venue_listing_column_status(connection)

    print(f"Existing venues: {venue_count}")
    print(f"Existing listing columns: {', '.join(sorted(existing_columns)) or 'none'}")
    ...
    with engine.begin() as connection:
        connection.execute(
            text("ALTER TABLE venues ADD COLUMN IF NOT EXISTS seat_capacity INTEGER")
        )
        connection.execute(
            text("ALTER TABLE venues ADD COLUMN IF NOT EXISTS amenity_tags TEXT")
        )
        connection.execute(
            text("ALTER TABLE venues ADD COLUMN IF NOT EXISTS rules_text TEXT")
        )
        for column in (
