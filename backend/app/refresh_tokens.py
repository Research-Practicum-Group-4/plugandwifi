...
REFRESH_TOKEN_EXPIRE_DAYS = get_positive_int_setting("REFRESH_TOKEN_EXPIRE_DAYS", 7)
...
def get_current_utc_naive_datetime():
    return datetime.now(timezone.utc).replace(tzinfo=None)


def issue_refresh_session(user_id, remember_me=False, family_id=None, expires_at=None):
    refresh_token = secrets.token_urlsafe(48)
    token_hash = hash_refresh_token(refresh_token)
    issued_at = get_current_utc_naive_datetime()
